...
final readonly class CsvOperationRunner
{
    ...
    {
        if ($operation->inputFileKey === null) throw ImportExportException::internal();
        $stream = $this->files->openCsvInput($context, $operation->inputFileKey);
        if (!is_resource($stream)) throw ImportExportException::fileUnavailable();
        $provider = $this->providers->require($operation->providerKey);
        ...
        while (($values = fgetcsv($stream, 1048577, ',', '"', '')) !== false) {
            ...
            if ($issues === []) {
                ...
                foreach ($providerIssues as $issue) {
                    if (!$issue instanceof RowIssue) throw ImportExportException::internal();
                    $issues[] = $issue;
                }
            }
            if ($issues !== []) {
                ...
                if ($issueCount > 10000) throw ImportExportException::limitExceeded();
                foreach ($issues as $issue) $this->repository->addRowIssue($operation->tenantId, $operation->id, $rowNumber, $issue);
            } else {
