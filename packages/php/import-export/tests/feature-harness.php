...
{
    try { $operation(); } catch (ImportExportException $exception) { check($code, $exception->problemCode, $label); return; }
...
final class HarnessFiles implements FileMediaGateway
{
    ...
    {
        if (!isset($this->inputs[$fileKey])) throw ImportExportException::fileUnavailable();
        $stream = fopen('php://temp', 'w+b'); fwrite($stream, $this->inputs[$fileKey]); rewind($stream); return $stream;
    }
    ...
    {
        $content = stream_get_contents($stream); if (!is_string($content)) throw ImportExportException::fileUnavailable();
        $key = 'file_' . substr(hash('sha256', $operationKey . '|' . $purpose), 0, 32); $this->outputs[$key] = $content; return $key;
    }
}
...
final class HarnessAudit implements AuditRepository
{
    ...
    public function appendPlatform(string $eventType, string $action, string $requestId, ?int $operatorId, ?int $accountId, array $metadata = []): void { throw new RuntimeException('platform audit not expected'); }
    public function appendTenantSystem(int $tenantId, string $eventType, string $action, string $requestId, array $metadata = []): void { throw new RuntimeException('system audit not expected'); }
    public function appendTenantMember(TenantContext $context, string $eventType, string $action, ?string $targetResourceType = null, ?string $targetResourceId = null, ?string $boundaryTargetType = null, ?string $boundaryTargetId = null, int $targetCount = 0, ?string $targetSetDigest = null, array $metadata = []): void { $this->events[] = $eventType . ':' . implode(',', array_keys($metadata)); }
    public function appendTenantPlatformOperator(int $tenantId, int $operatorId, int $accountId, string $eventType, string $action, string $requestId, array $metadata = []): void { throw new RuntimeException('platform audit not expected'); }
}
...
$pdo->exec("DROP DATABASE IF EXISTS `{$database}`"); $pdo->exec("CREATE DATABASE `{$database}` CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci"); $pdo->exec("USE `{$database}`");
...
$pdo->exec("INSERT INTO pa_tenant VALUES (101),(202)"); $pdo->exec("INSERT INTO pa_tenant_member VALUES (501,101,111,'active'),(502,202,212,'active')");
foreach (Schema::tableNames() as $table) $pdo->exec(Schema::createSql($table));

$repository = new PdoImportExportRepository($pdo); $provider = new HarnessProvider(); $files = new HarnessFiles(); $audit = new HarnessAudit(); $registry = new DataProviderRegistry([$provider]); $runner = new CsvOperationRunner($repository, $registry, $files, $audit);
$create101 = context(101, 501, 'create'); $read101 = context(101, 501, 'read');
...
$fileKey = 'file_' . str_repeat('a', 32); $files->inputs[$fileKey] = "Name,Email\r\nAlice,alice@example.test\r\nBob,invalid\r\n,empty@example.test\r\n";
...
check('succeeded', $import->status, 'import completion'); check(3, $import->processedRows, 'import progress'); check(1, $import->acceptedRows, 'accepted rows'); check(2, $import->rejectedRows, 'rejected rows'); check(1, count($provider->imported), 'provider called only for valid row');
check(2, count($repository->rowIssues(101, $import->id)), 'redacted row errors');
if ($import->errorFileKey === null || str_contains($files->outputs[$import->errorFileKey], 'invalid') || str_contains($files->outputs[$import->errorFileKey], 'empty@example')) throw new RuntimeException('Error report leaked rejected values.');
...
foreach (array_reverse(Schema::tableNames()) as $table) $pdo->exec(Schema::dropSql($table)); $pdo->exec('DROP TABLE pa_tenant_member, pa_tenant'); $pdo->exec("DROP DATABASE `{$database}`");
