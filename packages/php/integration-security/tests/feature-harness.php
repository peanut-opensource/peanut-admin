...
{
    try { $operation(); } catch (IntegrationSecurityException $exception) { same($code, $exception->problemCode, $message); return; }
...
function context(string $operation, int $tenantId = 101, int $accountId = 301, int $memberId = 501, string $sessionKey = '01J00000000000000000000000'): AuthorizedOperationContext
{
    ...
    return AuthorizedOperationContext::fromDecision(AuthorizationDecision::allow(
        TenantContext::fromValidatedSession($session, 'req_integration_feature'), Package::RESOURCE_KEY, $operation, [], hash('sha256', 'basis'),
    ));
}
...
final class MemoryRepository implements IntegrationSecurityRepository
{
    ...
    {
        if ($this->machine === null || !hash_equals($this->machine->identityKey, $identityKey) || $expectedRevision !== $this->machine->revision) throw IntegrationSecurityException::conflict();
        ...
        if ($this->auth !== null) $this->auth['status'] = 'revoked';
        return $this->machine;
    }
    ...
    {
        $this->endpoint = new WebhookEndpoint($endpointKey, $name, $url, $events, 'active', 1, '2026-07-24T10:00:00.000Z'); return $this->endpoint;
    }
    public function endpoints(int $tenantId): array { return $this->endpoint === null || $tenantId !== 101 ? [] : [$this->endpoint]; }
    ...
    {
        if ($this->endpoint === null || $expectedRevision !== 1) throw IntegrationSecurityException::conflict();
    ...
    {
        if ($this->endpoint === null) throw IntegrationSecurityException::endpointNotFound();
    ...
    public function activeEndpointKeysForEvent(int $tenantId, string $eventType): array { return $this->endpoint !== null && $tenantId === 101 && in_array($eventType, $this->endpoint->events, true) ? [['endpoint_key' => $this->endpoint->endpointKey]] : []; }
    public function enqueueDelivery(int $tenantId, string $endpointKey, TrustedWebhookEvent $event, DateTimeImmutable $now): string { ++$this->deliveryCount; return 'delivery_' . str_repeat('d', 32); }
    public function claimDelivery(int $tenantId, string $leaseDigest, int $leaseSeconds, DateTimeImmutable $now): ?WebhookDelivery { if ($this->claimed) return null; $this->claimed = true; return $this->delivery; }
...
$publicResolver = new class implements HostAddressResolver { public function resolve(string $host): array { return $host === 'hooks.example.com' ? ['93.184.216.34'] : ($host === 'mixed.example.com' ? ['93.184.216.34', '127.0.0.1'] : []); } };
...
$transport = new class($captured) implements WebhookTransport {
    ...
    public function __construct(& $captured) { $captured = &$this->request; }
    public function send(WebhookRequest $request): WebhookResponse { $this->request = $request; return new WebhookResponse(204, 12); }
};
...
truth($repository->completed, 'delivery completed'); truth($transport->request !== null, 'request captured');
