...
function truth(bool $condition, string $message): void { if (!$condition) throw new RuntimeException($message); }
...
function operation(string $name, int $tenantId, int $accountId, int $memberId, string $sessionKey): AuthorizedOperationContext
{
    ...
    return AuthorizedOperationContext::fromDecision(AuthorizationDecision::allow(
        TenantContext::fromValidatedSession($session, 'req_c03_mysql_' . $tenantId), Package::RESOURCE_KEY, $name, [], hash('sha256', 'basis-' . $name),
    ));
}

$dsn = getenv('C03_MYSQL_DSN'); $user = getenv('C03_MYSQL_USER'); $password = getenv('C03_MYSQL_PASSWORD');
if (!is_string($dsn) || $dsn === '' || !is_string($user) || !is_string($password)) throw new RuntimeException('C03 MySQL environment is incomplete.');
$serverDsn = preg_replace('/;dbname=[^;]*/i', '', $dsn);
if (!is_string($serverDsn)) throw new RuntimeException('C03 MySQL DSN is invalid.');
...
try {
    foreach ($drop as $table) dropTable($pdo, $table);
...
SQL);
    ...
    $session1 = '01J00000000000000000000000'; $sessionOther = '01J00000000000000000000001'; $sessionTenant2 = '01J00000000000000000000002';
    ...
    $scopeResolver = new class implements MachineScopeGrantResolver {
        public function grantableScopes(AuthorizedOperationContext $context): array { return $context->tenantContext->memberId === 501 ? ['data.export.read'] : []; }
    };
    ...
    same(hash('sha256', $machine->token), $stored, 'only token digest stored'); truth(!str_contains((string)$stored, $machine->token), 'plain token absent');
    $rotated = $machines->rotate(operation('machine-manage',101,301,501,$session1), $machine->identity->identityKey, 1);
    try { $machines->authenticate($machine->token, [], new DateTimeImmutable('2026-07-24T10:00:00Z')); throw new RuntimeException('old token survived rotation'); } catch (IntegrationSecurityException $exception) { same('MACHINE_TOKEN_INVALID',$exception->problemCode,'old token rejected'); }
    same(101, $machines->authenticate($rotated->token, [], new DateTimeImmutable('2026-07-24T10:00:00Z'))->tenantId, 'rotated token active');

    $resolver = new class implements HostAddressResolver { public function resolve(string $host): array { return $host === 'hooks.example.com' ? ['93.184.216.34'] : []; } };
    $policy = new WebhookDestinationPolicy($resolver); $protector = new AesGcmWebhookSecretProtector('mysql-key', base64_encode(str_repeat('m',32)));
    ...
    $publisher = new TrustedWebhookPublisher($repository); $event = new TrustedWebhookEvent('event:2026:00000001','audit.event.created',['id'=>42]);
    $first = $publisher->publish(101,$event,new DateTimeImmutable('2026-07-24T10:00:00Z')); $second = $publisher->publish(101,$event,new DateTimeImmutable('2026-07-24T10:01:00Z'));
    same($first,$second,'delivery idempotent'); same(1,(int)$pdo->query('SELECT COUNT(*) FROM pa_integration_webhook_delivery')->fetchColumn(),'one delivery row');
    $transport = new class implements WebhookTransport { public ?WebhookRequest $last = null; public function send(WebhookRequest $request): WebhookResponse { $this->last=$request; return new WebhookResponse(204,9); } };
    $dispatcher = new WebhookDispatcher($repository,$policy,$protector,$transport); truth($dispatcher->runOne(101,new DateTimeImmutable('2026-07-24T10:02:00Z')),'delivery dispatched');
    same('delivered',(string)$pdo->query('SELECT status FROM pa_integration_webhook_delivery')->fetchColumn(),'delivery terminal'); same(1,(int)$pdo->query('SELECT COUNT(*) FROM pa_integration_webhook_attempt')->fetchColumn(),'attempt logged');
    truth(preg_match('/^v1=[0-9a-f]{64}$/D',$transport->last?->headers['X-Peanut-Signature']??'')===1,'signature emitted'); same(false,$transport->last?->followRedirects,'redirect disabled');
    ...
    same($retryKey, $recovered?->deliveryKey, 'expired lease below eight retryable and reclaimed'); same(8, $recovered?->attemptNumber, 'reclaimed delivery advances attempt');
    ...
    if ($recovered === null) throw new RuntimeException('expired lease recovery missing');
    ...
    same('permanent_failed', $expired['status'], 'attempt eight permanently failed'); same(null, $expired['lease_digest'], 'expired lease digest cleared'); same(null, $expired['lease_expires_at'], 'expired lease expiry cleared');
    ...
    $sessions = new SessionSecurityService($repository); $listed = $sessions->list(operation('session-read',101,301,501,$session1));
    same(1,count($listed),'sessions self scoped'); same('203.0.113.*',$listed[0]->maskedIp,'IP masked');
    try { $sessions->revoke(operation('session-revoke',101,301,501,$session1),$sessionOther); throw new RuntimeException('cross-account session revoked'); } catch (IntegrationSecurityException $exception) { same('SESSION_DEVICE_NOT_FOUND',$exception->problemCode,'cross account hidden'); }
    ...
    foreach ($drop as $table) { try { dropTable($pdo, $table); } catch (Throwable) {} }
    try { $selected = guardedDatabase($pdo); $pdo->exec("DROP DATABASE `{$selected}`"); } catch (Throwable) {}
}
