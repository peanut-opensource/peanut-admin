...
final readonly class MachineIdentityService
{
    ...
    {
        $name = trim($name);
        if ($name === '' || mb_strlen($name) > 120 || ($expiresAt !== null && $expiresAt <= new DateTimeImmutable('now'))) throw IntegrationSecurityException::invalid();
    ...
    private function scopes(array $scopes, bool $allowEmpty): array
    {
        ...
        foreach ($scopes as $scope) {
            if (!is_string($scope) || preg_match('/^[a-z][a-z0-9]*(?:[.-][a-z0-9]+)+$/D', $scope) !== 1 || strlen($scope) > 96) throw IntegrationSecurityException::invalid();
            $unique[$scope] = true;
        }
        if ((!$allowEmpty && $unique === []) || count($unique) > 32) throw IntegrationSecurityException::invalid();
        $result = array_keys($unique); sort($result, SORT_STRING); return $result;
    }
    ...
    {
        if (preg_match('/^machine_[0-9a-f]{32}$/D', $key) !== 1) throw IntegrationSecurityException::machineNotFound();
    }
