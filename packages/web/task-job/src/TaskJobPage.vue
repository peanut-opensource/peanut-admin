...
<template>
  <PageContent class="task-job-page">
    ...
    <div v-else-if="state.loading" class="task-state" role="status">Loading tasks...</div>
    <EmptyState v-else-if="state.items.length === 0" title="No tasks" message="No tasks match this status." />
      ...
      <table class="task-table">
        ...
        <tbody>
          <tr v-for="job in state.items" :key="job.jobKey">
            ...
            <td>
              <ElButton v-if="job.status === 'queued'" text :disabled="!canManage || state.mutating" @click="runtime.cancel(job)">Cancel</ElButton>
              <ElButton v-if="job.status === 'dead'" text :disabled="!canManage || state.mutating" @click="runtime.retry(job)">Retry</ElButton>
            </td>
          </tr>
        </tbody>
      </table>
