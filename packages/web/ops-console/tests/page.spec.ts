...
describe('ops-console page', () => {
  it('shows operational evidence while keeping ungranted actions disabled', async () => {
    const runtime = createOpsConsoleRuntime({
      ...
      canRead: () => true, canBackup: () => false, canRestore: () => false, canMaintain: () => false, canReadLogs: () => false,
    })
    ...
    expect(wrapper.text()).toContain('UPGRADE_PREFLIGHT_READY'); expect(wrapper.text()).toContain('You do not have permission to read runtime events.')
