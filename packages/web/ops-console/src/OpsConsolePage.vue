...
<template>
  <PageContent class="ops-console-page">
    <PageHeader>
      Operations
      <template #actions><ElButton :loading="state.loading" :disabled="state.mutating" @click="runtime.load">Reload</ElButton></template>
    </PageHeader>

    <SessionExpiredState v-if="state.error?.status === 401" :message="state.error.message" />
    <ForbiddenState v-else-if="state.error?.status === 403 && state.overview === null" :message="state.error.message" />
    <ModuleUnavailableState v-else-if="state.error?.status === 503 && state.overview === null" :message="state.error.message" @action="runtime.load" />
    <section v-else-if="state.error && state.overview === null" role="alert" class="ops-state">
      <h2>Unable to load operations</h2><p>{{ state.error.message }}</p><p v-if="state.error.requestId">Request ID: {{ state.error.requestId }}</p>
    </section>
    <div v-else-if="state.loading && state.overview === null" class="ops-state" role="status">Loading operations...</div>
    ...
    <ElTabs v-else-if="state.overview !== null" class="ops-tabs">
      <ElTabPane label="Overview">
        <section class="ops-section" aria-labelledby="ops-health-heading">
          ...
          <dl class="ops-facts">
            ...
            <div><dt>Commit</dt><dd class="mono">{{ state.overview.version.commit }}</dd></div>
            <div><dt>Tree</dt><dd class="mono">{{ state.overview.version.tree }}</dd></div>
          ...
          <div class="table-wrap">
            <table><thead><tr><th>Check</th><th>Status</th><th>Critical</th><th>Latency</th></tr></thead>
              <tbody><tr v-for="check in state.overview.health.checks" :key="check.key"><td>{{ check.key }}</td><td>{{ check.status }}</td><td>{{ check.critical ? 'yes' : 'no' }}</td><td>{{ check.latencyMs }} ms</td></tr></tbody>
            </table>
          </div>
        </section>
      </ElTabPane>
      ...
      <ElTabPane label="Recovery">
        <section class="ops-section" aria-labelledby="ops-backup-heading">
          ...
          <div class="ops-controls">
            <label>Provider<select v-model="providerKey" :disabled="state.mutating" @change="chooseProvider"><option v-for="provider in runtime.providers" :key="provider.key" :value="provider.key">{{ provider.key }}</option></select></label>
            <ElButton type="primary" :disabled="!runtime.canBackup() || state.mutating || !runtime.providers.find(provider => provider.key === providerKey)?.backup" @click="runtime.submitBackup(providerKey)">Create backup</ElButton>
          </div>
          <div class="ops-controls">
            <label>Backup reference<ElInput v-model="backupReferenceKey" :disabled="state.mutating" /></label>
            <label>New target<select v-model="restoreTargetKey" :disabled="state.mutating"><option v-for="target in targets" :key="target" :value="target">{{ target }}</option></select></label>
            <ElButton :disabled="!runtime.canRestore() || state.mutating || restoreTargetKey === ''" @click="runtime.submitRestore(providerKey, backupReferenceKey, restoreTargetKey)">Restore and verify</ElButton>
          </div>
          <section v-if="state.error" role="alert" class="inline-error"><p>{{ state.error.message }}</p><p v-if="state.error.requestId">Request ID: {{ state.error.requestId }}</p></section>
          <EmptyState v-if="state.tasks.length === 0" title="No operation tasks" message="No backup or restore-verification tasks were submitted in this session." />
          <div v-else class="table-wrap"><table><thead><tr><th>Type</th><th>Status</th><th>Attempts</th><th>Updated</th><th>Action</th></tr></thead>
            <tbody><tr v-for="task in state.tasks" :key="task.taskKey"><td>{{ task.taskType }}</td><td>{{ task.status }}</td><td>{{ task.attemptCount }} / {{ task.maxAttempts }}</td><td>{{ task.updatedAt }}</td><td><ElButton text @click="runtime.refreshTask(task)">Refresh</ElButton></td></tr></tbody>
          </table></div>
        </section>
      </ElTabPane>
      ...
      <ElTabPane label="Maintenance">
        <section class="ops-section" aria-labelledby="ops-maintenance-heading">
          ...
          <dl v-if="state.maintenance" class="ops-facts"><div><dt>State</dt><dd>{{ state.maintenance.state }}</dd></div><div><dt>Reason</dt><dd>{{ state.maintenance.reasonKey }}</dd></div><div><dt>Starts</dt><dd>{{ state.maintenance.startsAt }}</dd></div><div><dt>Ends</dt><dd>{{ state.maintenance.endsAt }}</dd></div><div><dt>Revision</dt><dd>{{ state.maintenance.revision }}</dd></div></dl>
          <div class="ops-controls">
            <label>Reason<select v-model="reasonKey" :disabled="state.mutating"><option v-for="reason in runtime.maintenanceReasons" :key="reason" :value="reason">{{ reason }}</option></select></label>
            <label>Starts<ElDatePicker v-model="startsAt" type="datetime" value-format="YYYY-MM-DDTHH:mm:ss.SSS[Z]" /></label>
            <label>Ends<ElDatePicker v-model="endsAt" type="datetime" value-format="YYYY-MM-DDTHH:mm:ss.SSS[Z]" /></label>
            <ElButton type="primary" :disabled="!runtime.canMaintain() || state.mutating || reasonKey === '' || startsAt === '' || endsAt === ''" @click="schedule">{{ state.maintenance === null ? 'Schedule' : 'Replace' }}</ElButton>
            <ElButton :disabled="!runtime.canMaintain() || state.mutating || !activeMaintenance" @click="runtime.closeMaintenance">Close</ElButton>
          </div>
        </section>
      </ElTabPane>
      ...
      <ElTabPane label="Runtime events">
        <section class="ops-section" aria-labelledby="ops-logs-heading">
          ...
          <template v-else>
            <div class="ops-controls"><label>Source<select v-model="draftLogSource" aria-label="Source"><option v-for="source in runtime.logSources" :key="source" :value="source">{{ source }}</option></select></label><label>Severity<select v-model="draftLogSeverity" aria-label="Severity"><option v-for="severity in LOG_SEVERITIES" :key="severity" :value="severity">{{ severity }}</option></select></label><ElButton :loading="state.logsLoading" @click="applyLogFilter">Apply</ElButton></div>
            ...
            <ElButton v-if="state.logNextCursor" :loading="state.logsLoading" @click="runtime.loadLogs(false)">Load more</ElButton>
          </template>
        </section>
      </ElTabPane>
    </ElTabs>
