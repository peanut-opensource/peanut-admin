...
const transport: FileMediaTransport = {
  async list(status, page, pageSize, signal) {
    return transportResult(await (await apiClient()).GET('/api/v1/files', {
  ...
  async download(fileKey, signal) {
    return (await (await apiClient()).GET('/api/v1/files/{file_key}/content', {
      params: { path: { file_key: fileKey } }, parseAs: 'blob', signal,
    })).response
  },
  async archive(fileKey, etag, signal) {
    return transportResult(await (await apiClient()).DELETE('/api/v1/files/{file_key}', {
...
export const loadFileMediaRoute = async () => {
...
export const peanutFileMediaModule = defineAdminModule({
