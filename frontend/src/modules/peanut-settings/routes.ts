...
import type { components } from '@peanut-admin/admin-core'
import {
...
const transport: SettingsTransport = {
  async list(signal) {
    return transportResult(await apiClient().GET('/api/v1/settings', { signal }))
  },
  async replace(moduleKey, settingKey, request) {
    return transportResult(await apiClient().PUT('/api/v1/settings/{module_key}/{setting_key}', {
  ...
  async unset(moduleKey, settingKey, request) {
    return transportResult(await apiClient().DELETE('/api/v1/settings/{module_key}/{setting_key}', {
      params: {
...
export const peanutSettingsModule = defineAdminModule({
