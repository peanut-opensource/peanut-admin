import { existsSync } from 'node:fs'
import { describe, expect, it } from 'vitest'

describe('internal starter Notification/SMS consumption', () => {
  it('composes the inbox route with no real provider call', async () => {
    expect(existsSync(new URL('../src/modules/peanut-notification-sms.ts', import.meta.url))).toBe(true)
    const { createStarterModules } = await import('../src/app/modules')
    const host = createStarterModules({
      baseUrl: 'https://starter.example.test', canRead: () => true, canManage: () => false,
      fetch: async () => new Response(JSON.stringify({
        data: { items: [] }, meta: { request_id: 'req_notifications', page: 1, page_size: 20, total: 0 },
      }), { status: 200, headers: { 'Content-Type': 'application/json' } }),
    })
    expect(host.modules.map(module => module.key)).toEqual([
      'example.greeting', 'peanut.settings', 'peanut.reference-codes', 'peanut.file-media', 'peanut.task-job', 'peanut.notification-sms', 'peanut.import-export', 'peanut.integration-security',
    ])
    expect(host.notificationSmsModule.routes[0]).toMatchObject({ name: 'peanut.notification-sms.inbox', path: '/app/notifications' })
    await host.notificationSmsRuntime.load()
    expect(host.notificationSmsRuntime.state.items).toEqual([])
    host.notificationSmsRuntime.dispose()
  })
})
