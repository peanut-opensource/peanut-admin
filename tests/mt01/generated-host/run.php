...
function migrationManager(PDO $pdo, string $path, string $environment, string $table): Manager
{
    $database = $pdo->query('SELECT DATABASE()');
    if ($database === false) {
        throw new RuntimeException('Generated Host database context is unavailable.');
    }
    ...
    return new Manager(new Config([
        ...
        'environments' => [
            ...
            $environment => [
                'adapter' => 'mysql',
                'connection' => $pdo,
                'name' => (string) $database->fetchColumn(),
...
$moduleMigrations = migrationManager($pdo, $migrationRoot, 'fixture_record', 'pa_fixture_record_migration');
$moduleMigrations->migrate('fixture_record');
...
foreach ($denials as $name => [$denialHost, $denialContext, $scope, $expectedCode]) {
    ...
    $response = $denialHost->command(
        $operation,
        FixtureRecordHost::request(
            $denialContext,
            $denialContext->requestId,
            ['name' => 'Denied'],
            [target($scope)],
            "MT01-DENIED-{$name}-0001",
        ),
...
$moduleMigrations->rollback('fixture_record', 'all', true);
