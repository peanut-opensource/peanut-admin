...
final readonly class TenantMemberDirectoryProvider implements DataProvider
{
    ...
    public function key(): string { return 'tenant.member-directory'; }
    ...
    {
        if($this->validateImport($context,$row)!==[])throw ImportExportException::schemaMismatch();
        ...
        if($statement->rowCount()>1)throw ImportExportException::internal();
    }
    public function exportBatch(AuthorizedOperationContext $context,?string $cursor,int $limit):ExportBatch
    {
        ...
        if(!is_int($after)||$limit<1||$limit>500)throw ImportExportException::invalid();
