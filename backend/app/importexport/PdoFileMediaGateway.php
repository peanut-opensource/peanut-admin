...
final readonly class PdoFileMediaGateway implements FileMediaGateway
{
    ...
    {
        try{return $this->service()->content($context->tenantContext,$fileKey);}catch(Throwable){throw ImportExportException::fileUnavailable();}
    }
    ...
    {
        if(!is_resource($stream)||!in_array($purpose,['result','errors'],true))throw ImportExportException::fileUnavailable();
        $temporary=tempnam(sys_get_temp_dir(),'peanut-iox-');if(!is_string($temporary))throw ImportExportException::fileUnavailable();
        try{$output=fopen($temporary,'wb');if(!is_resource($output))throw ImportExportException::fileUnavailable();try{if(!is_int(stream_copy_to_stream($stream,$output))||!fflush($output))throw ImportExportException::fileUnavailable();}finally{fclose($output);}return $this->service()->upload($context->tenantContext,$temporary,$filename)->fileKey;}catch(Throwable $e){if($e instanceof ImportExportException)throw $e;throw ImportExportException::fileUnavailable();}finally{@unlink($temporary);}
    }
