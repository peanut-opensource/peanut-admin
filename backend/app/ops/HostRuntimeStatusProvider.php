...
final readonly class HostRuntimeStatusProvider implements RuntimeStatusProvider
{
    ...
    public function snapshot(PlatformContext $context):OpsStatusSnapshot
    {
        ...
        $repository=(new RepositoryInspector())->inspect($this->root);$migrations=$this->pdo->query("SELECT module_key,migration_key,checksum,status FROM pa_module_migration ORDER BY module_key,migration_key")->fetchAll(PDO::FETCH_ASSOC);$applied=count(array_filter($migrations,static fn(array $r):bool=>$r['status']==='applied'));$digest=hash('sha256',json_encode($migrations,JSON_THROW_ON_ERROR|JSON_UNESCAPED_SLASHES));$upgrade=UpgradeStatusService::fromEnvironment()->status();$target=is_array($upgrade['target']??null)?$upgrade['target']:[];$preflight=is_array($upgrade['preflight']??null)?$upgrade['preflight']:[];$backup=is_array($upgrade['backup']??null)?$upgrade['backup']:[];
