...
final class TaskWorkerCommand extends Command
{
    ...
    {
        $tenant=filter_var($input->getOption('tenant'),FILTER_VALIDATE_INT,['options'=>['min_range'=>1]]);$worker=(string)$input->getOption('worker');
        if(!is_int($tenant))throw new \InvalidArgumentException('A positive --tenant is required.');
        $status=NotificationRuntimeFactory::worker(MemberAdminRuntime::pdo(),$tenant,$worker)->runOnce();$output->writeln($status??'idle');return 0;
    }
}
