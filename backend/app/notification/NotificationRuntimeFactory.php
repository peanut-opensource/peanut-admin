...
final class NotificationRuntimeFactory
{
    ...
    {
        $config=self::config();$recipients=new PdoRecipientResolver($pdo,$config['recipient_directory'],$config['recipient_digest_key']);
    ...
    public static function worker(PDO $pdo,int $tenantId,string $workerId): LocalWorker
    {
        $config=self::config();
    ...
    {
        $key=self::config()['envelope_key'];if(strlen($key)<32)throw new RuntimeException('TASK_ENVELOPE_KEY_UNAVAILABLE');return new TrustedEnvelopeCodec($key);
    }

    /** @return array{provider:string,envelope_key:string,recipient_digest_key:string,recipient_directory:array<string,mixed>} */
    private static function config(): array
    {
        $config=require dirname(__DIR__,3).'/backend/config/notification-sms.php';
        if(!is_array($config)||!is_string($config['provider']??null)||!is_string($config['envelope_key']??null)||!is_string($config['recipient_digest_key']??null)||!is_array($config['recipient_directory']??null))throw new RuntimeException('NOTIFICATION_CONFIG_INVALID');
        return $config;
    }
}
