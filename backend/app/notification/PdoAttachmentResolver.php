...
final readonly class PdoAttachmentResolver implements AttachmentResolver
{
    ...
    {
        if (preg_match('/^file_[0-9a-f]{32}$/D', $fileKey) !== 1) throw NotificationException::attachmentUnavailable();
...
SQL);
        ...
        if (!is_array($row)) throw NotificationException::attachmentUnavailable();
        return new AttachmentReference(
            $fileKey, (string) $row['original_name'], (string) $row['media_type'],
            (int) $row['size_bytes'], (string) $row['sha256'],
        );
    }
}
