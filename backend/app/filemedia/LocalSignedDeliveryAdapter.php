...
final readonly class LocalSignedDeliveryAdapter implements DeliveryAdapter
{
    ...
    {
        if(preg_match('#^https://[a-z0-9](?:[a-z0-9.-]*[a-z0-9])?$#D',$baseUrl)!==1)throw new \InvalidArgumentException('FILE_DELIVERY_BASE_URL_INVALID');
    }
    public function key(): string{return 'local-signed';}
    public function supportsStorageProvider(string $providerKey):bool{return hash_equals('local-private',$providerKey);}
    public function issue(DeliveryRequest $request):DeliveryGrant
    {
        $tokenId=bin2hex(random_bytes(16));$token=$this->tokens->issue($request->context->tenantId,$request->file->fileKey,$request->visibility,$request->replayMode,$request->issuedAt,$request->ttlSeconds,$tokenId);
