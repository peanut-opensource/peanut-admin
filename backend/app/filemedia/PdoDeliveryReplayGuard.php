...
final readonly class PdoDeliveryReplayGuard implements ReplayGuard
{
    ...
    {
        if ($this->tenantId < 1 || preg_match('/^[0-9a-f]{32}$/D', $tokenId) !== 1 || $expiresAt <= $now) return false;
        ...
        try{$statement->execute(['tenant_id'=>$this->tenantId,'token_hash'=>hash('sha256',$tokenId),'expires_at'=>$expiresAt->setTimezone($utc)->format('Y-m-d H:i:s.v'),'consumed_at'=>$now->setTimezone($utc)->format('Y-m-d H:i:s.v')]);return true;}
