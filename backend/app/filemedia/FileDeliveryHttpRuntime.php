...
final class FileDeliveryHttpRuntime
{
    ...
    {
        $pdo=MemberAdminRuntime::pdo();$modules=RuntimeModuleRegistry::compile();$op=TenantModuleRuntime::operation('listFileAssets','GET','/api/v1/file-assets','peanut.file-media','peanut.file-media.read');$external=TenantModuleRuntime::request($request,$op,'/api/v1/file-assets');
        $response=TenantModuleRuntime::host($pdo,$modules)->read($op,$external,static function($authorized,$query)use($pdo){try{self::emptyBody($query->body['payload']??null);$raw=$query->body['query']??null;if(!is_array($raw)||array_diff(array_keys($raw),['page','page_size'])!==[])throw FileMediaException::deliveryInvalid();$context=TenantModuleRuntime::context($authorized);$result=(new FileDeliveryRepository($pdo))->assets($context->tenantId,TenantModuleRuntime::positiveInt($raw['page']??'1',10000),TenantModuleRuntime::positiveInt($raw['page_size']??'20',100));$items=[];foreach($result['items'] as $item){$file=(new PdoFileRepository($pdo))->get($context->tenantId,(string)$item['file_key']);$grant=self::service($pdo,$context->tenantId)->issue(new DeliveryRequest($context,$file,DeliveryVisibility::Private,ReplayMode::SingleUse,$query->comparisonTime,120,true));$items[]=['file_key'=>$file->fileKey,'original_name'=>$file->originalName,'media_type'=>$item['media_type'],'width'=>(int)$item['width'],'height'=>(int)$item['height'],'preview_uri'=>$grant->uri,'variants'=>[]];}return new ExternalOperationResponse(200,['data'=>['items'=>$items],'page'=>$result['page'],'page_size'=>$result['page_size'],'total'=>$result['total']]);}catch(FileMediaException $e){throw self::problem($e);}});
    ...
    {
        $pdo=MemberAdminRuntime::pdo();$modules=RuntimeModuleRegistry::compile();$path='/api/v1/files/'.rawurlencode($fileKey).'/delivery-grants';$op=TenantModuleRuntime::operation('createFileDeliveryGrant','POST','/api/v1/files/{file_key}/delivery-grants','peanut.file-media','peanut.file-media.read',true);$external=TenantModuleRuntime::request($request,$op,$path);
        $response=TenantModuleRuntime::host($pdo,$modules)->command($op,$external,static function($authorized,$command,PDO $transaction)use($fileKey){try{self::noInput($command->body);$context=TenantModuleRuntime::context($authorized);$file=(new PdoFileRepository($transaction))->get($context->tenantId,$fileKey);$grant=self::service($transaction,$context->tenantId)->issue(new DeliveryRequest($context,$file,DeliveryVisibility::Private,ReplayMode::SingleUse,$command->comparisonTime,120,true));return new \PeanutAdmin\Kernel\Host\ExternalOperationResult(201,['data'=>['file_key'=>$fileKey,'delivery_uri'=>$grant->uri,'visibility'=>$grant->visibility->value,'replay_mode'=>$grant->replayMode->value,'expires_at'=>$grant->expiresAt->setTimezone(new DateTimeZone('UTC'))->format('Y-m-d\TH:i:s\Z')]],'tenant.file.delivery.granted','peanut.file-media.read',$grant->auditMetadata(),'file',$fileKey);}catch(FileMediaException $e){throw self::problem($e);}},guard:TenantModuleRuntime::commandGuard('peanut.file-media'));
        return TenantModuleRuntime::response($response,$external->requestId->value);
    }
    ...
    private static function service(PDO $pdo,int $tenantId): DeliveryService{return new DeliveryService(self::adapter($pdo,$tenantId),new DeliveryPolicy());}
    private static function adapter(PDO $pdo,int $tenantId):LocalSignedDeliveryAdapter{$c=self::config();if($c['provider']!=='local-private'||$c['delivery_adapter']!=='local-signed')throw FileMediaException::deliveryUnavailable();return new LocalSignedDeliveryAdapter($c['delivery_base_url'],self::tokens($pdo,$tenantId));}
    private static function tokens(PDO $pdo,int $tenantId):SignedDeliveryTokenService{$key=self::config()['delivery_signing_key'];if(strlen($key)<32)throw FileMediaException::deliveryUnavailable();return new SignedDeliveryTokenService($key,new PdoDeliveryReplayGuard($pdo,$tenantId));}
    private static function storage():PrivateStorageAdapter{$c=self::config();if($c['provider']!=='local-private')throw FileMediaException::storageUnavailable();return new PrivateStorageAdapter(new LocalPrivateStorageProvider($c['local_root'],$c['public_roots']));}
