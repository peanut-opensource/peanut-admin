...
final class TaskHttpRuntime
{
    ...
    {
        $pdo = MemberAdminRuntime::pdo(); $modules = RuntimeModuleRegistry::compile();
        ...
        $response = TenantModuleRuntime::host($pdo, $modules)->read($operation, $external, static function ($authorized, $query) use ($pdo) {
            try {
                ...
                if (!is_array($raw) || array_diff(array_keys($raw), ['status','page','page_size']) !== []) throw TaskJobException::invalid();
    ...
    public static function cancel(Request $request, string $jobKey): Response { return self::mutate($request, $jobKey, 'cancelTask'); }
    public static function retry(Request $request, string $jobKey): Response { return self::mutate($request, $jobKey, 'retryTask'); }

    private static function readOne(Request $request, string $jobKey): Response
    {
        $pdo = MemberAdminRuntime::pdo(); $modules = RuntimeModuleRegistry::compile(); $path='/api/v1/tasks/'.rawurlencode($jobKey);
        ...
        $response=TenantModuleRuntime::host($pdo,$modules)->read($operation,$external,static function($authorized,$query)use($pdo,$jobKey){
            try { self::noInput($query->body); $job=(new TaskJobService(new PdoTaskJobRepository($pdo)))->detail(TenantModuleRuntime::authorizedContext($authorized, 'peanut.task-job', 'read'),$jobKey); return new \PeanutAdmin\Kernel\Host\ExternalOperationResponse(200,['data'=>$job->toPublicArray()]); }
            catch(TaskJobException $e){throw self::problem($e);}
        });
        return TenantModuleRuntime::response($response,$external->requestId->value);
    }
    ...
    {
        $pdo=MemberAdminRuntime::pdo();$modules=RuntimeModuleRegistry::compile();$suffix=$operationId==='cancelTask'?'cancel':'retry';$path='/api/v1/tasks/'.rawurlencode($jobKey).'/'.$suffix;
